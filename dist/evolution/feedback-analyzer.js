...
exports.getFeedbackAnalyzer = getFeedbackAnalyzer;
...
class FeedbackAnalyzer {
    ...
    calculateSatisfaction(feedback) {
        ...
        const weights = types_1.EVOLUTION_RULES.feedbackWeights;
        // Analyze reaction
        if (feedback.reaction) {
            const reactionScore = types_1.REACTION_SCORES[feedback.reaction] || 0;
        ...
        logger_1.log.debug('[FeedbackAnalyzer] Satisfaction calculated', {
     ...
     */
    analyzeFollowUp(followUp) {
        const lowerText = followUp.toLowerCase();
        // Check for positive indicators
        const positiveCount = types_1.POSITIVE_FOLLOWUP_WORDS.filter(word => lowerText.includes(word)).length;
        // Check for negative indicators
        const negativeCount = types_1.NEGATIVE_FOLLOWUP_WORDS.filter(word => lowerText.includes(word)).length;
     ...
     */
    extractCorrectionPattern(originalResponse, correction) {
     ...
     */
    analyzeResponseTime(responseTimeMs) {
     ...
     */
    analyzeResponseLength(response, userInput) {
     ...
     */
    generateFeedbackSummary(feedbacks) {
        ...
        const negativeReactions = feedbacks
            .filter(f => f.reaction && (types_1.REACTION_SCORES[f.reaction] || 0) < 0)
            .map(f => f.reaction);
...
// Export singleton
let feedbackAnalyzerInstance = null;
function getFeedbackAnalyzer() {
