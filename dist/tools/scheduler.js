...
exports.SCHEDULER_TOOLS = void 0;
 ...
 * Schedule a task
 */
async function scheduleTask(input, userId) {
    ...
    try {
        ...
        logger_1.log.info('[Scheduler] Task scheduled', {
            id: job.id,
            name: job.name,
            when: job.expression
        });
...
Use \`cancel_scheduled_task\` with ID to cancel.`;
        ...
        logger_1.log.error('[Scheduler] Failed to schedule task', { error: error.message });
 ...
 */
async function listScheduledTasks(input, userId) {
    try {
        const cronManager = (0, cron_manager_1.getCronManager)();
        ...
        logger_1.log.error('[Scheduler] Failed to list tasks', { error: error.message });
 ...
 */
async function cancelScheduledTask(input) {
    const { task_id } = input;
    if (!task_id) {
    ...
    try {
        ...
        if (deleted) {
            logger_1.log.info('[Scheduler] Task cancelled', { taskId: task_id });
        ...
        logger_1.log.error('[Scheduler] Failed to cancel task', { error: error.message });
