...
exports.managedProcesses = exports.PROCESS_TOOLS = void 0;
exports.executeProcessTool = executeProcessTool;
const child_process_1 = require("child_process");
const logger_1 = require("../logger");
// Store managed processes
const managedProcesses = new Map();
exports.managedProcesses = managedProcesses;
exports.PROCESS_TOOLS = [
    {
        name: 'process_start',
...
async function executeProcessTool(toolName, input) {
    try {
        switch (toolName) {
            case 'process_start':
...
async function processStart(input) {
    ...
    try {
        ...
        if (proc.stdout) {
            proc.stdout.on('data', (data) => {
                logger_1.log.info(`[Process:${name}] ${data.toString().trim()}`);
            });
        }
        if (proc.stderr) {
            proc.stderr.on('data', (data) => {
                logger_1.log.error(`[Process:${name}] ${data.toString().trim()}`);
            });
        }
        // Handle process exit
        proc.on('exit', (code, signal) => {
            logger_1.log.info(`Process "${name}" exited`, { code, signal });
            if (auto_restart && code !== 0) {
                managed.restartCount++;
                logger_1.log.warn(`Restarting process "${name}" (attempt ${managed.restartCount})...`);
                // Wait 2 seconds before restart
                setTimeout(() => {
                    if (managedProcesses.has(name)) {
                        managedProcesses.delete(name);
                        processStart({ name, command, auto_restart }).then(result => {
                            logger_1.log.info(`Process "${name}" restarted: ${result}`);
                        });
                    }
                }, 2000);
            }
...
async function processList(input) {
    if (managedProcesses.size === 0) {
        return 'No managed processes running.';
    }
    const lines = ['Managed Processes:', ''];
...
async function processStop(input) {
...
async function processRestart(input) {
    const { name } = input;
    const managed = managedProcesses.get(name);
    if (!managed) {
        return `Process "${name}" not found. Use process_list to see running processes.`;
...
async function processLogs(input) {
