...
exports.ELEVENLABS_TOOLS = void 0;
exports.executeElevenLabsTool = executeElevenLabsTool;
...
function getElevenLabsClient() {
    if (!elevenLabsClient) {
        ...
        elevenLabsClient = new elevenlabs_js_1.ElevenLabsClient({ apiKey });
    }
    return elevenLabsClient;
}
exports.ELEVENLABS_TOOLS = [
    {
...
async function executeElevenLabsTool(toolName, input) {
    try {
        ...
        logger_1.log.error(`[ElevenLabs] Tool execution failed: ${toolName}`, { error: error.message });
        return `Error: ${error.message}`;
    }
}
async function textToSpeech(client, input) {
    const { text, voice = 'rachel', model = 'eleven_multilingual_v2' } = input;
    // Get voice ID
    const voiceId = VOICE_MAP[voice.toLowerCase()] || voice;
    logger_1.log.info('[ElevenLabs] Generating speech', {
        voice,
    ...
    try {
        ...
        const outputDir = path_1.default.join(process.env.DATA_DIR || './data', 'audio');
        ...
        logger_1.log.info('[ElevenLabs] Speech generated successfully', {
            filepath,
            size: `${fileSize}KB`
        });
        ...
        logger_1.log.error('[ElevenLabs] Speech generation failed', { error: error.message });
        throw error;
    }
}
async function listVoices(client) {
    logger_1.log.info('[ElevenLabs] Listing voices');
    try {
        ...
        logger_1.log.info('[ElevenLabs] Listed voices', { count: voices.length });
        return `Available Voices (${voices.length}):\n\n${voiceList}`;
    }
    catch (error) {
        logger_1.log.error('[ElevenLabs] Failed to list voices', { error: error.message });
        throw error;
    }
}
async function getVoiceInfo(client, input) {
    const { voice_id } = input;
    // Check if it's a name, convert to ID
    const voiceId = VOICE_MAP[voice_id.toLowerCase()] || voice_id;
    logger_1.log.info('[ElevenLabs] Getting voice info', { voiceId });
    try {
...
`.trim();
        logger_1.log.info('[ElevenLabs] Retrieved voice info', { name: voice.name });
        return info;
    }
    catch (error) {
        logger_1.log.error('[ElevenLabs] Failed to get voice info', { error: error.message });
        throw error;
    }
}
