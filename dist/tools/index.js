...
async function executeTool(toolName, toolInput, userId, userRequest) {
    ...
    try {
        ...
        const result = await (0, tool_executor_1.executeToolSecurely)(toolName, userId || 'unknown', async () => {
 ...
 */
async function executeToolInternal(toolName, toolInput, userId) {
    let result;
    // Base tools
    switch (toolName) {
        case 'execute_shell':
        ...
        case 'deploy_public_app':
            const { webHost } = await Promise.resolve().then(() => __importStar(require('./web-host')));
        ...
        case 'list_public_apps':
            const { webHost: webHostList } = await Promise.resolve().then(() => __importStar(require('./web-host')));
        ...
        case 'delete_public_app':
            const { webHost: webHostDelete } = await Promise.resolve().then(() => __importStar(require('./web-host')));
