...
exports.sendQRCodeToDiscord = sendQRCodeToDiscord;
exports.sendWhatsAppStatusToDiscord = sendWhatsAppStatusToDiscord;
 ...
 */
async function sendQRCodeToDiscord(qrData, discordClient) {
    if (!discordClient) {
    ...
    try {
        ...
        const qrImageBuffer = await qrcode_1.default.toBuffer(qrData, {
        ...
        const attachment = new discord_js_1.AttachmentBuilder(qrImageBuffer, { name: 'whatsapp-qr.png' });
        ...
        logger_1.log.info('[WhatsApp] QR Code sent to Discord', { channelId });
    }
    catch (error) {
        logger_1.log.error('[WhatsApp] Failed to send QR to Discord', {
            error: error.message,
            channelId
        });
        throw error;
 ...
 */
async function sendWhatsAppStatusToDiscord(discordClient, status, details) {
    if (!discordClient)
        return;
    const channelId = process.env.WHATSAPP_QR_CHANNEL_ID || process.env.DISCORD_CHANNEL_ID;
    if (!channelId)
    ...
    try {
        ...
        logger_1.log.info('[WhatsApp] Status sent to Discord', { status });
    }
    catch (error) {
        logger_1.log.error('[WhatsApp] Failed to send status to Discord', { error: error.message });
    }
}
