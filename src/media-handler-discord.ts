  ...
  try {
    ...
    if (media.url) {
      ...
      if (media.type === 'image' || media.type === 'video') {
        try {
          await message.reply({
            content: text || `✨ Generated ${media.type}`,
            embeds: [{
      ...
      if (fileBuffer.length > DISCORD_FILE_LIMIT) {
        await message.reply(
          `⚠️ File too large for Discord (${(fileBuffer.length / 1024 / 1024).toFixed(2)}MB > 25MB limit)\n\n` +
    ...
    // For local files (ElevenLabs audio)
      ...
      if (fileBuffer.length > DISCORD_FILE_LIMIT) {
        await message.reply(
          `⚠️ Audio file too large for Discord (${(fileBuffer.length / 1024 / 1024).toFixed(2)}MB > 25MB limit)`
        );
        return;
      }
