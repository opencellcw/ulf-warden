...
export const OPENAI_TOOLS: Anthropic.Tool[] = [
  {
...
export async function executeOpenAITool(toolName: string, input: any): Promise<string> {
  try {
    ...
    log.error(`[OpenAI] Tool execution failed: ${toolName}`, { error: error.message });
    return `Error: ${error.message}`;
  }
}

async function generateImage(client: OpenAI, input: any): Promise<string> {
  const {
    prompt,
    model = 'dall-e-3',
    size = '1024x1024',
  ...
  log.info('[OpenAI] Generating image with DALL-E', { model, size });
  ...
  try {
    ...
    log.info('[OpenAI] Image generated successfully', { url: imageUrl });

    return `✅ Image generated with ${model.toUpperCase()}!\n\nURL: ${imageUrl}\n\nPrompt: ${prompt}\nSize: ${size}\nQuality: ${quality}`;
  } catch (error: any) {
    log.error('[OpenAI] Image generation failed', { error: error.message });
    throw error;
  }
}

async function gptChat(client: OpenAI, input: any): Promise<string> {
  const {
    prompt,
    model = 'gpt-4-turbo',
    max_tokens = 1000,
  ...
  log.info('[OpenAI] Calling GPT', { model, promptLength: prompt.length });
  ...
  try {
    ...
    log.info('[OpenAI] GPT response received', {
      model,
    ...
    log.error('[OpenAI] GPT call failed', { error: error.message });
    throw error;
  }
}

async function transcribeAudio(client: OpenAI, input: any): Promise<string> {
  const { file_url, file_path, language } = input;

  log.info('[OpenAI] Transcribing audio with Whisper');
  ...
  try {
    ...
    log.info('[OpenAI] Audio transcribed successfully', {
      textLength: transcription.text.length
    });

    return `✅ Audio transcribed successfully!\n\nTranscription:\n${transcription.text}\n\n---\nLanguage: ${language || 'auto-detected'}`;
  } catch (error: any) {
    log.error('[OpenAI] Transcription failed', { error: error.message });
    throw error;
  }
}

async function analyzeImage(client: OpenAI, input: any): Promise<string> {
  const { image_url, prompt = 'Describe this image in detail' } = input;

  log.info('[OpenAI] Analyzing image with GPT-4 Vision');
  ...
  try {
    ...
    log.info('[OpenAI] Image analyzed successfully');

    return `✅ Image Analysis:\n\n${analysis}\n\n---\nImage: ${image_url}`;
  } catch (error: any) {
    log.error('[OpenAI] Image analysis failed', { error: error.message });
    throw error;
  }
}
