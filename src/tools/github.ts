...
export async function executeGitHubTool(toolName: string, input: any): Promise<string> {
  try {
    switch (toolName) {
      case 'github_clone':
...
async function githubClone(input: any): Promise<string> {
  ...
  try {
    // Try git clone
    const output = execSync(`git clone ${repoUrl} ${targetPath}`, {
      encoding: 'utf-8',
...
async function githubSearch(input: any): Promise<string> {
...
async function githubIssue(input: any): Promise<string> {
  ...
  try {
    ...
    switch (action) {
      case 'create':
        if (!title) return 'Title is required to create an issue';
        command = `gh issue create --repo ${repo} --title "${title}" --body "${body || ''}"`;
        break;
      case 'view':
        if (!issue_number) return 'Issue number is required to view an issue';
...
async function githubPr(input: any): Promise<string> {
  ...
  try {
    ...
    switch (action) {
      case 'create':
        ...
        break;
      case 'view':
        if (!pr_number) return 'PR number is required to view a PR';
