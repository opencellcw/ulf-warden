...
import { FeedbackData, Pattern, LearningInsight, EVOLUTION_RULES } from './types';
...
export class PatternRecognition {
   ...
   */
  async identifyPatterns(interactions: FeedbackData[]): Promise<Pattern[]> {
    if (interactions.length < EVOLUTION_RULES.learningTriggers.minInteractionsForPattern) {
      log.info('[PatternRecognition] Not enough interactions', {
        count: interactions.length,
        required: EVOLUTION_RULES.learningTriggers.minInteractionsForPattern
      });
      return [];
    }
    ...
    log.info('[PatternRecognition] Patterns identified', {
   ...
   */
  private identifyLowSatisfactionTopics(interactions: FeedbackData[]): Pattern[] {
    const patterns: Pattern[] = [];
    ...
    for (const [topic, scores] of Object.entries(topicScores)) {
      if (scores.length < 3) continue; // Need at least 3 samples

      const avgScore = scores.reduce((a, b) => a + b, 0) / scores.length;

      if (avgScore < EVOLUTION_RULES.learningTriggers.satisfactionThresholdLow) {
        const examples = interactions.filter(i =>
          this.feedbackAnalyzer.extractTopic(i.userInput) === topic
        ).slice(0, 3);
   ...
   */
  private identifyHighSatisfactionPatterns(interactions: FeedbackData[]): Pattern[] {
    const patterns: Pattern[] = [];
    const highSatisfactionInteractions = interactions.filter(
      i => i.satisfactionScore >= EVOLUTION_RULES.learningTriggers.satisfactionThresholdHigh
    );

    if (highSatisfactionInteractions.length >= 5) {
      // Analyze what makes these interactions successful
      const topics = highSatisfactionInteractions.map(i =>
        this.feedbackAnalyzer.extractTopic(i.userInput)
      );
   ...
   */
  private identifyCommonCorrections(interactions: FeedbackData[]): Pattern[] {
    const patterns: Pattern[] = [];
    const corrections = interactions.filter(i =>
      i.followUp && this.feedbackAnalyzer.isCorrection(i.followUp)
    );

    if (corrections.length >= EVOLUTION_RULES.learningTriggers.correctionFrequencyAlert) {
      // Extract correction patterns
      const correctionPatterns: Record<string, FeedbackData[]> = {};

      for (const correction of corrections) {
        const pattern = this.feedbackAnalyzer.extractCorrectionPattern(
          correction.ulfResponse,
          correction.followUp!
        );
   ...
   */
  private identifyToneFeedback(interactions: FeedbackData[]): Pattern[] {
    const patterns: Pattern[] = [];
    ...
    const toneFeedback = interactions.filter(i =>
      i.followUp && toneRelatedKeywords.some(kw =>
        i.followUp!.toLowerCase().includes(kw)
      )
    );
   ...
   */
  async generateInsights(patterns: Pattern[]): Promise<LearningInsight[]> {
    ...
    log.info('[PatternRecognition] Insights generated', {
   ...
   */
  recommendImprovements(patterns: Pattern[]): string[] {
      ...
      switch (pattern.type) {
        case 'low_satisfaction':
          recommendations.push(
            `📚 Estudar mais sobre "${pattern.topic}" para melhorar respostas`
          );
          recommendations.push(
            `🔍 Buscar exemplos de boas respostas sobre "${pattern.topic}"`
          );
          break;
        ...
        case 'high_satisfaction':
          recommendations.push(
            `✨ Replicar abordagem de "${pattern.topic}" para outros tópicos`
          );
          break;
      }
    }
