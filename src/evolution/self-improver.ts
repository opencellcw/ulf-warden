...
export class SelfImprover {
  ...
  async proposeImprovement(idea: string): Promise<ImprovementProposal> {
    log.info('[SelfImprover] Analyzing improvement idea', { idea: idea.substring(0, 100) });
...
Return ONLY a JSON object (no markdown, no explanation):
{
  ...
  "estimatedChanges": 50
    ...
    const validation = validateProposal(proposalData.files, stats.todayProposed);
    if (!validation.valid) {
      throw new Error(validation.reason);
    }

    const risk = assessRisk(proposalData.files);
    const id = uuidv4();
    ...
    log.info('[SelfImprover] Improvement proposed', {
      id: proposal.id,
  ...
  async implementProposal(proposal: ImprovementProposal): Promise<void> {
    log.info('[SelfImprover] Implementing proposal', { id: proposal.id });
    ...
    try {
      ...
      log.info('[SelfImprover] Created branch', { branch: proposal.branch });
...
Generate complete TypeScript code. Return ONLY JSON:
{
  "files": {
    "src/commands/status.ts": "export function status() { return 'OK'; }"
      ...
      // Write files
      for (const [filePath, fileContent] of Object.entries(files) as [string, string][]) {
        const dir = path.dirname(filePath);
        if (!fs.existsSync(dir)) {
          fs.mkdirSync(dir, { recursive: true });
        }
        fs.writeFileSync(filePath, fileContent);
        log.info('[SelfImprover] Created file', { file: filePath });
      }

      // Build
      await execAsync('npm run build');
      log.info('[SelfImprover] Build successful');
...
Proposal ID: ${proposal.id}"`);
      ...
      log.info('[SelfImprover] Pushed branch');
      ...
      log.info('[SelfImprover] Implementation complete', { prUrl: proposal.prUrl });

    } catch (error: any) {
      log.error('[SelfImprover] Implementation failed', { error: error.message });
  ...
  private async createPullRequest(proposal: ImprovementProposal): Promise<number> {
    const requiredApprovals = getRequiredApprovals(proposal.risk);
...
⚠️ **IMPORTANT**: This is an auto-generated PR. Review carefully before merging.`;
  ...
  async requestApproval(proposal: ImprovementProposal, channel: string): Promise<void> {
    ...
    log.info('[SelfImprover] Approval requested', { proposalId: proposal.id, channel });
  }

  async handleApproval(proposalId: string, approved: boolean, userId: string): Promise<void> {
    ...
    if (approved) {
      ...
      if (approvedBy.length >= requiredApprovals) {
        ...
        log.info('[SelfImprover] Proposal approved', { proposalId, approvals: approvedBy.length });
      } else {
        this.updateProposal(proposalId, { approvedBy });
        log.info('[SelfImprover] Approval recorded', {
          proposalId,
      ...
      log.info('[SelfImprover] Proposal rejected', { proposalId, by: userId });
    }
  }

  async deploy(proposal: ImprovementProposal): Promise<void> {
    if (proposal.status !== 'approved') {
      throw new Error('Only approved proposals can be deployed');
    }

    log.info('[SelfImprover] Deploying proposal', { id: proposal.id });
    ...
    try {
      ...
      log.info('[SelfImprover] Deployed', { id: proposal.id, prNumber: proposal.prNumber });
    } catch (error: any) {
      log.error('[SelfImprover] Deployment failed', { error: error.message });
      throw error;
    }
  }

  getProposal(id: string): ImprovementProposal | null {
    const row = this.db.prepare('SELECT * FROM improvement_proposals WHERE id = ?').get(id) as any;
    if (!row) return null;
  ...
  private saveProposal(proposal: ImprovementProposal): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      proposal.reasoning, proposal.risk, JSON.stringify(proposal.files),
      proposal.branch, proposal.prUrl || null, proposal.prNumber || null,
      proposal.status, proposal.proposedBy, proposal.proposedAt,
      JSON.stringify(proposal.approvedBy || []), proposal.rejectedBy || null,
      proposal.deployedAt || null, proposal.implementationPlan,
      proposal.estimatedChanges, JSON.stringify(proposal.dependencies || []),
      proposal.attempts, JSON.stringify(proposal.errors || [])
    );
  }

  private updateProposal(id: string, updates: Partial<ImprovementProposal>): void {
    const sets: string[] = [];
    const values: any[] = [];
  ...
  private rowToProposal(row: any): ImprovementProposal {
    return {
      id: row.id,
      type: row.type,
      title: row.title,
