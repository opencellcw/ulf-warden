...
import { HeartbeatState, HeartbeatConfig, HeartbeatCheck, DEFAULT_HEARTBEAT_CONFIG } from './types';
...
export class HeartbeatManager {
  ...
  constructor(app: App, claude: Anthropic, workspace: WorkspaceLoader, config?: Partial<HeartbeatConfig>) {
    this.app = app;
    this.claude = claude;
    this.workspace = workspace;
    this.config = { ...DEFAULT_HEARTBEAT_CONFIG, ...config };
    this.stateFile = path.join(process.env.DATA_DIR || './data', 'heartbeat-state.json');
  ...
  private loadState(): HeartbeatState {
    try {
      ...
      log.error('[Heartbeat] Failed to load state', { error: error.message });
    }
  ...
  private saveState(): void {
    try {
      ...
      log.error('[Heartbeat] Failed to save state', { error: error.message });
    }
  }
   ...
   */
  private async performCheck(): Promise<HeartbeatCheck> {
    const timestamp = new Date().toISOString();

    log.info('[Heartbeat] Performing check', { timestamp });
    ...
    try {
      ...
      log.info('[Heartbeat] Check response', {
      ...
      log.error('[Heartbeat] Check failed', { error: error.message });
...
Be concise. Only alert if truly important.`;
   ...
   * Send alert to Slack
   */
  private async sendAlert(check: HeartbeatCheck): Promise<void> {
    ...
    if (this.isQuietHours()) {
      log.info('[Heartbeat] Skipping alert (quiet hours)', {
    ...
    try {
      ...
      log.info('[Heartbeat] Alert sent to Slack', {
        channel: this.config.slackChannel
      });
    } catch (error: any) {
      log.error('[Heartbeat] Failed to send alert', {
        error: error.message,
        channel: this.config.slackChannel
      });
    }
  ...
  start(): void {
    if (!this.config.enabled) {
      log.info('[Heartbeat] Not starting (disabled in config)');
      return;
    }
    ...
    this.interval = setInterval(async () => {
      try {
        ...
        log.error('[Heartbeat] Loop error', { error: error.message });
      }
    }, intervalMs);
   ...
   */
  async trigger(): Promise<HeartbeatCheck> {
    log.info('[Heartbeat] Manual trigger');
    const check = await this.performCheck();
    await this.sendAlert(check);
    return check;
  }
...
export function getHeartbeatManager(
  app?: App,
  claude?: Anthropic,
